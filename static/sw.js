...
// Critical PWA fix: Ensure proper content type for HTML responses
self.addEventListener('fetch', (event) => {
  const { request } = event;
  const url = new URL(request.url);
  ...
  // Special handling for PWA standalone mode
  if (url.pathname === '/' || url.pathname === '/dashboard') {
    event.respondWith(handlePWAPageRequest(request));
    return;
  }
...
// Special handler for PWA main pages
async function handlePWAPageRequest(request) {
  try {
    const response = await fetch(request);
    if (response.ok) {
      // Ensure proper content type for PWA pages
